...
// Logo loaded at runtime — avoids large inline base64 in bundle
const getYrcLogoBase64 = (): string => {
  try {
    if (typeof window === 'undefined') {
      const fs = require('fs'), path = require('path')
      const p = path.join(process.cwd(), 'public', 'yrclogo.jpg')
      if (fs.existsSync(p)) return `data:image/jpeg;base64,${fs.readFileSync(p).toString('base64')}`
    }
  } catch {}
  return ''
}
const yrcLogoBase64 = getYrcLogoBase64()
